...
******************************************************************************/
#ifndef SOFA_COMPONENT_MASS_PatternPlusFileActivation_INL
#define SOFA_COMPONENT_MASS_PatternPlusFileActivation_INL

#include "PatternPlusFileActivation.h"
...
PatternPlusFileActivation<DataTypes, MassType>::PatternPlusFileActivation()
	...
	, f_patternConstant(initData(&f_patternConstant,(Real)10.0,"pattern_scale","scale the CPG"))
	, f_fileConstant(initData(&f_fileConstant,(Real)10.0,"file_scale","scale the activation forces from file"))
	, f_activationPeriod(initData(&f_activationPeriod,(Real)1.0,"activation_period","period of the activation forces 1"))
	...
	, f_activationPhase(initData(&f_activationPhase,(Real)0,"activation_phase","activation phase"))
{
    f_graph.setWidget("graph");
	f_fileNodes.setGroup("Forces");
	f_fileActivation.setGroup("Forces");
	f_musclePositionFile.setGroup("Forces");
	f_patternConstant.setGroup("Forces");
	f_fileConstant.setGroup("Forces");
	f_activationPeriod.setGroup("Forces");
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyCreateFunction(unsigned int, MassType & VertexMass,
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyCreateFunction(unsigned int, MassType & EdgeMass,
        const topology::Edge&,
        const sofa::helper::vector< unsigned int > &,
        const sofa::helper::vector< double >&)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::TetrahedronMassHandler::applyCreateFunction(unsigned int tetra, MassVector & TetrahedronMass,
        const topology::Tetrahedron&,
        const sofa::helper::vector< unsigned int > &,
        const sofa::helper::vector< double >&)
{
	PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;

	if (MMM && (MMM->bezierTetraGeo) && (MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_BEZIERTETRAHEDRONSET))
	{	
		...
		if (MMM->integrationMethod==PatternPlusFileActivation<DataTypes, MassType>::NUMERICAL_INTEGRATION) {
	...
	//		std::cerr<<"Lumped Mass Matrix= "<<lumpedVertexMass<<std::endl;
		...
		} else if ((MMM->integrationMethod==PatternPlusFileActivation<DataTypes, MassType>::AFFINE_ELEMENT_INTEGRATION) || 
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyTriangleCreation(const sofa::helper::vector< unsigned int >& triangleAdded,
        ...
        const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/)
{
    ...
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_TRIANGLESET)
    {
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyTriangleCreation(const sofa::helper::vector< unsigned int >& triangleAdded,
        const sofa::helper::vector< topology::Triangle >& /*elems*/,
        const sofa::helper::vector< sofa::helper::vector< unsigned int > >& /*ancestors*/,
        const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyTriangleDestruction(const sofa::helper::vector< unsigned int >& triangleRemoved)
{
    PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_TRIANGLESET)
    {
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyTriangleDestruction(const sofa::helper::vector< unsigned int >& triangleRemoved)
{
    PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_TRIANGLESET)
    {
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::ApplyTopologyChange(const core::topology::TrianglesAdded* e)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::ApplyTopologyChange(const core::topology::TrianglesRemoved* e)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::ApplyTopologyChange(const core::topology::TrianglesAdded* e)
{
    const sofa::helper::vector<unsigned int> &triangleAdded = e->getIndexArray();
    const sofa::helper::vector<topology::Triangle> &elems = e->getElementArray();
    const sofa::helper::vector<sofa::helper::vector<unsigned int> > & ancestors = e->ancestorsList;
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::ApplyTopologyChange(const core::topology::TrianglesRemoved* e)
{
    const sofa::helper::vector<unsigned int> &triangleRemoved = e->getArray();
...
/// Creation fonction for mass stored on vertices
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyQuadCreation(const sofa::helper::vector< unsigned int >& quadAdded,
        ...
        const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/)
{
    ...
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_QUADSET)
    {
        helper::WriteAccessor< Data< vector<MassType> > > VertexMasses ( MMM->vertexMassInfo );
        // Initialisation
        typename DataTypes::Real densityM = MMM->getMassDensity();
...
/// Creation fonction for mass stored on edges
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyQuadCreation(const sofa::helper::vector< unsigned int >& quadAdded,
        const sofa::helper::vector< topology::Quad >& /*elems*/,
        const sofa::helper::vector< sofa::helper::vector< unsigned int > >& /*ancestors*/,
        const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/)
{
...
/// Destruction fonction for mass stored on vertices
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyQuadDestruction(const sofa::helper::vector< unsigned int >& quadRemoved)
{
    PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_QUADSET)
    {
...
/// Destruction fonction for mass stored on edges
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyQuadDestruction(const sofa::helper::vector< unsigned int >& quadRemoved)
{
    PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_QUADSET)
    {
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::ApplyTopologyChange(const core::topology::QuadsAdded* e)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::ApplyTopologyChange(const core::topology::QuadsRemoved* e)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::ApplyTopologyChange(const core::topology::QuadsAdded* e)
{
    const sofa::helper::vector<unsigned int> &quadAdded = e->getIndexArray();
    const sofa::helper::vector<topology::Quad> &elems = e->getElementArray();
    const sofa::helper::vector<sofa::helper::vector<unsigned int> > & ancestors = e->ancestorsList;
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::ApplyTopologyChange(const core::topology::QuadsRemoved* e)
{
    const sofa::helper::vector<unsigned int> &quadRemoved = e->getArray();
...
/// Creation fonction for mass stored on vertices
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyTetrahedronCreation(const sofa::helper::vector< unsigned int >& tetrahedronAdded,
        ...
        const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/)
{
    ...
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_TETRAHEDRONSET)
    {
        helper::WriteAccessor< Data< vector<MassType> > > VertexMasses ( MMM->vertexMassInfo );
        // Initialisation
        typename DataTypes::Real densityM = MMM->getMassDensity();
...
/// Creation fonction for mass stored on edges
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyTetrahedronCreation(const sofa::helper::vector< unsigned int >& tetrahedronAdded,
        const sofa::helper::vector< topology::Tetrahedron >& /*elems*/,
        const sofa::helper::vector< sofa::helper::vector< unsigned int > >& /*ancestors*/,
        const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/)
{
...
/// Destruction fonction for mass stored on vertices
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyTetrahedronDestruction(const sofa::helper::vector< unsigned int >& tetrahedronRemoved)
{
    PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_TETRAHEDRONSET)
    {
...
/// Destruction fonction for mass stored on edges
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyTetrahedronDestruction(const sofa::helper::vector< unsigned int >& tetrahedronRemoved)
{
    PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_TETRAHEDRONSET)
    {
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::ApplyTopologyChange(const core::topology::TetrahedraAdded* e)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::ApplyTopologyChange(const core::topology::TetrahedraRemoved* e)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::ApplyTopologyChange(const core::topology::TetrahedraAdded* e)
{
    const sofa::helper::vector<unsigned int> &tetraAdded = e->getIndexArray();
    const sofa::helper::vector<topology::Tetrahedron> &elems = e->getElementArray();
    const sofa::helper::vector<sofa::helper::vector<unsigned int> > & ancestors = e->ancestorsList;
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::ApplyTopologyChange(const core::topology::TetrahedraRemoved* e)
{
    const sofa::helper::vector<unsigned int> &tetraRemoved = e->getArray();
...
/// Creation fonction for mass stored on vertices
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyHexahedronCreation(const sofa::helper::vector< unsigned int >& hexahedronAdded,
        ...
        const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/)
{
    ...
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_HEXAHEDRONSET)
    {
        helper::WriteAccessor< Data< vector<MassType> > > VertexMasses ( MMM->vertexMassInfo );
        // Initialisation
        typename DataTypes::Real densityM = MMM->getMassDensity();
...
/// Creation fonction for mass stored on edges
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyHexahedronCreation(const sofa::helper::vector< unsigned int >& hexahedronAdded,
        const sofa::helper::vector< topology::Hexahedron >& /*elems*/,
        const sofa::helper::vector< sofa::helper::vector< unsigned int > >& /*ancestors*/,
        const sofa::helper::vector< sofa::helper::vector< double > >& /*coefs*/)
{
...
/// Destruction fonction for mass stored on vertices
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::applyHexahedronDestruction(const sofa::helper::vector< unsigned int >& hexahedronRemoved)
{
    PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_HEXAHEDRONSET)
    {
...
/// Destruction fonction for mass stored on edges
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::applyHexahedronDestruction(const sofa::helper::vector< unsigned int >& hexahedronRemoved)
{
    PatternPlusFileActivation<DataTypes, MassType> *MMM = this->m;
    if (MMM && MMM->getMassTopologyType()==PatternPlusFileActivation<DataTypes, MassType>::TOPOLOGY_HEXAHEDRONSET)
    {
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::ApplyTopologyChange(const core::topology::HexahedraAdded* e)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::VertexMassHandler::ApplyTopologyChange(const core::topology::HexahedraRemoved* e)
{
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::ApplyTopologyChange(const core::topology::HexahedraAdded* e)
{
    const sofa::helper::vector<unsigned int> &hexaAdded = e->getIndexArray();
    const sofa::helper::vector<topology::Hexahedron> &elems = e->getElementArray();
    const sofa::helper::vector<sofa::helper::vector<unsigned int> > & ancestors = e->ancestorsList;
...
template< class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::EdgeMassHandler::ApplyTopologyChange(const core::topology::HexahedraRemoved* e)
{
    const sofa::helper::vector<unsigned int> &hexaRemoved = e->getArray();
...
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::init()
{
	...
	m_patternConstant = f_patternConstant.getValue();
	m_fileConstant = f_fileConstant.getValue();
	m_activationPeriod = f_activationPeriod.getValue();
	m_activationPhase = f_activationPhase.getValue();
...
template <class DataTypes, class MassType>
bool PatternPlusFileActivation<DataTypes, MassType>::loadMuscleExtremes(const char* filename){
...
template <class DataTypes, class MassType>
std::vector<std::string> &PatternPlusFileActivation<DataTypes, MassType>::split(std::string &s, char delim, std::vector<std::string> &elems) {
...
template <class DataTypes, class MassType>
std::vector<std::string> PatternPlusFileActivation<DataTypes, MassType>::split(std::string &s, char delim) {
...
template <class DataTypes, class MassType>
bool PatternPlusFileActivation<DataTypes, MassType>::loadActivationOrderFile(const char* filename){
	/*std::string meshFilename(filename);
	if (!sofa::helper::system::DataRepository.findFile (meshFilename))
	{
		serr << "Mesh \""<< filename <<"\" not found"<< sendl;
...
template <class DataTypes, class MassType>
bool PatternPlusFileActivation<DataTypes, MassType>::loadSignalFile(const char* filename){
	/*std::string meshFilename(filename);
	if (!sofa::helper::system::DataRepository.findFile (meshFilename))
	{
		serr << "Mesh \""<< filename <<"\" not found"<< sendl;
...
template <class DataTypes, class MassType>
bool PatternPlusFileActivation<DataTypes, MassType>::loadMusclePositions(const char* filename){
	/*std::string meshFilename(filename);
	if (!sofa::helper::system::DataRepository.findFile (meshFilename))
	{
		serr << "Mesh \""<< filename <<"\" not found"<< sendl;
...
template <class DataTypes, class MassType>
bool PatternPlusFileActivation<DataTypes, MassType>::loadForceNodeFile(const char* filename)
{
	/*std::string meshFilename(filename);
	if (!sofa::helper::system::DataRepository.findFile (meshFilename))
	{
...
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::reinit()
{
...
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::copyVertexMass() {}


template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::clear()
{
...
// -- Mass interface
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::addMDx(const core::MechanicalParams*, DataVecDeriv& vres, const DataVecDeriv& vdx, SReal factor)
{
...
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::accFromF(const core::MechanicalParams*, DataVecDeriv& a, const DataVecDeriv& f)
{
    ...
    else
    {
        (void)a;
        (void)f;
        serr << "WARNING: the methode 'accFromF' can't be used with PatternPlusFileActivation as this SPARSE mass matrix can't be inversed easily. \nPlease proceed to mass lumping." << sendl;
        return;
    }
}
...
#ifdef SOFA_SUPPORT_MOVING_FRAMES
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::addForce(const core::MechanicalParams*, DataVecDeriv& vf, const DataVecCoord& vx, const DataVecDeriv& vv)
{
...
#else
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::addForce(const core::MechanicalParams* /*mp*/, DataVecDeriv& vf, const DataVecCoord& d_x, const DataVecDeriv& )
{
		helper::WriteAccessor< DataVecDeriv > f = vf;
	...
	if (!f_signalFile.getValue().empty())
	{
				...
				for(unsigned int k = 0; k < m_orderedMuscles.at(i).at(j).size(); k++){
					if(k < muscleMidRef){
						for(unsigned int l = 0; l < m_orderedMuscles.at(i).at(j).at(k).size(); l++){
							if(m_activationSignals.size() > startingTimestep){
								if(l == 0 && k == 0){
									m_saveSignals.at(i).at(j) = currentSignals.at(i).at(j)*(muscleMidRef-k)*m_fileConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								}
								f[m_orderedMuscles[i][j][k][l]][0] += currentSignals.at(i).at(j)*(muscleMidRef-k)*m_fileConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]][1] += currentSignals.at(i).at(j)*(muscleMidRef-k)*m_fileConstant*(x[m_orderedMuscles[i][j][k+1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]][2] += currentSignals.at(i).at(j)*(muscleMidRef-k)*m_fileConstant*(x[m_orderedMuscles[i][j][k+1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);
							}

							f[m_orderedMuscles[i][j][k][l]](0) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
							f[m_orderedMuscles[i][j][k][l]](1) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
							f[m_orderedMuscles[i][j][k][l]](2) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);
						...
						for(unsigned int l = 0; l < m_orderedMuscles.at(i).at(j).at(k).size(); l++){
							if(m_activationSignals.size() > startingTimestep){
								f[m_orderedMuscles[i][j][k][l]][0] += currentSignals.at(i).at(j)*(k-muscleMidRef)*m_fileConstant*(x[m_orderedMuscles[i][j][k-1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]][1] += currentSignals.at(i).at(j)*(k-muscleMidRef)*m_fileConstant*(x[m_orderedMuscles[i][j][k-1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]][2] += currentSignals.at(i).at(j)*(k-muscleMidRef)*m_fileConstant*(x[m_orderedMuscles[i][j][k-1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);
							}
							//if(m_orderedMuscles[i][j][k][l] == 331 || m_orderedMuscles[i][j][k][l] == 348) std::cout << "f3: " << counter << " ; " << f[331] << " , " << f[348] << std::endl;

							f[m_orderedMuscles[i][j][k][l]](0) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
							f[m_orderedMuscles[i][j][k][l]](1) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
							f[m_orderedMuscles[i][j][k][l]](2) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);
						}
					}	
				}
			}
		...
		aux.push_back(x[0][0] - x[295][0]);
		aux.push_back(x[0][2] - x[295][2]);
		for(unsigned int i = 0; i < f.size(); i++){
			f[i][0] += 10*aux.at(0);
			f[i][2] += 10*aux.at(1);
		}
				...
				for(unsigned int k = 0; k < m_orderedMuscles.at(i).at(j).size(); k++){
					if(k < muscleMidRef){
						for(unsigned int l = 0; l < m_orderedMuscles.at(i).at(j).at(k).size(); l++){
							...
							if(j < 4){
								/*f[m_orderedMuscles[i][j][k][l]][0] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 +sign[i]*0.1)*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]][1] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 +sign[i]*0.1)*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]][2] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 +sign[i]*0.1)*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);*/
								f[m_orderedMuscles[i][j][k][l]](0) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]](1) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]](2) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);
								if(j == 0 && i == 0 && k == 0 && l == 0){
									std::cout << "a: " << (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]) << "\n";
								}
								if(j == 0 && i == 4 && k == 0 && l == 0){
									std::cout << "b: " << (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]) << "\n";
								}
							}
							else{
								/*f[m_orderedMuscles[i][j][k][l]][0] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]][1] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]][2] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);*/
								f[m_orderedMuscles[i][j][k][l]](0) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]](1) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]](2) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(muscleMidRef-k)*m_patternConstant*(x[m_orderedMuscles[i][j][k+1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);
						...
						for(unsigned int l = 0; l < m_orderedMuscles.at(i).at(j).at(k).size(); l++){
							...
							if(j < 4){
								/*f[m_orderedMuscles[i][j][k][l]][0] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 +sign[i]*0.1)*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]][1] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 +sign[i]*0.1)*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]][2] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 +sign[i]*0.1)*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);*/
								f[m_orderedMuscles[i][j][k][l]](0) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]](1) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]](2) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);
								if(j == 0 && i == 0 && k == 0 && l == 0){
									std::cout << "b: " << (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 -sign[i]*0.1)*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]) << "\n";
								}
							}
							else{
								/*f[m_orderedMuscles[i][j][k][l]][0] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]][1] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]][2] += (float)( 0.4f * (sin(3*pi*j/24 + sign[i]*pi/2 + pi/2 - 0.5*counter*direction)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);*/
								f[m_orderedMuscles[i][j][k][l]](0) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][0]-x[m_orderedMuscles[i][j][k][l]][0]);
								f[m_orderedMuscles[i][j][k][l]](1) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][1]-x[m_orderedMuscles[i][j][k][l]][1]);
								f[m_orderedMuscles[i][j][k][l]](2) += (float)( 0.4f * (sin(m_activationPhase + 3*pi*j/24 + sign[i]*pi/2 + pi/2 - 2*pi*counter*direction/m_activationPeriod)+1.f)/2 )*(k-muscleMidRef)*m_patternConstant*(x[m_orderedMuscles[i][j][k-1][l]][2]-x[m_orderedMuscles[i][j][k][l]][2]);
							}
...
template <class DataTypes, class MassType>
SReal PatternPlusFileActivation<DataTypes, MassType>::getKineticEnergy( const core::MechanicalParams*, const DataVecDeriv& vv ) const
{
    const MassVector &vertexMass= vertexMassInfo.getValue();
    const MassVector &edgeMass= edgeMassInfo.getValue();
...
template <class DataTypes, class MassType>
SReal PatternPlusFileActivation<DataTypes, MassType>::getPotentialEnergy( const core::MechanicalParams*, const DataVecCoord& vx) const
{
    const MassVector &vertexMass= vertexMassInfo.getValue();
...
template <class DataTypes, class MassType>
defaulttype::Vector6 PatternPlusFileActivation<DataTypes, MassType>::getMomentum ( const core::MechanicalParams*, const DataVecCoord& /*vx*/, const DataVecDeriv& /*vv*/  ) const
{
...
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::addGravityToV(const core::MechanicalParams* mparams, DataVecDeriv& d_v)
{
...
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::addMToMatrix(const core::MechanicalParams *mparams, const sofa::core::behavior::MultiMatrixAccessor* matrix)
{
    const MassVector &vertexMass= vertexMassInfo.getValue();
    const MassVector &edgeMass= edgeMassInfo.getValue();
...
template <class DataTypes, class MassType>
SReal PatternPlusFileActivation<DataTypes, MassType>::getElementMass(unsigned int index) const
{
    const MassVector &vertexMass= vertexMassInfo.getValue();
...
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::getElementMass(unsigned int index, defaulttype::BaseMatrix *m) const
{
...
template <class DataTypes, class MassType>
void PatternPlusFileActivation<DataTypes, MassType>::draw(const core::visual::VisualParams* vparams)
{
